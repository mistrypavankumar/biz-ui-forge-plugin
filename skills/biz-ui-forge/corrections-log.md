...
### CR-021 — Put scroll on the outer body Box, causing the table header to scroll away with rows
- **Date**: 2026-05-18
- **Status**: active
- **Category**: skipped-state
- **Mode**: fix
- **What happened**: When user asked to cap the modal list at 7 rows then make it scrollable, I added `maxHeight: 460, overflowY: 'auto'` to the OUTER body Box in `restore-deleted-records-modal.tsx` that wraps `<RecordsTable>`. Because the list head and the rows both live inside `RecordsTable`'s `list-wrap`, scrolling the outer body moved head + rows together. The user's next screenshot showed the header had scrolled off-screen under the policy callout — sticky-header behavior was missing.
- **Root cause**: I put the scroll boundary at the wrong layer. For sticky-header table behavior, the scroll container must be the table itself (or the wrapper that directly contains both the head and the rows), and the head must be `position: sticky; top: 0` inside that scroll container. Scrolling the parent of the whole table scrolls everything as one block, which is the wrong UX for a tabular layout with a labeled header. Additional sub-cause: `list-wrap` had `overflow: 'hidden'` (to clip rounded corners), which would have prevented `position: sticky` from working inside it anyway because `overflow: hidden` creates a containing block for sticky and the wrap itself doesn't scroll.
- **Correct behavior**: For table-like lists with a header row and a "show N rows then scroll" cap: put `maxHeight: <px>, overflowY: 'auto', overflowX: 'hidden'` directly on the list wrapper (the element that contains both the head and the rows). Make the head `position: 'sticky', top: 0, zIndex: 1` with an OPAQUE bg (use `background.neutral` or `background.paper` — NOT `alpha(...)` over the wrap's bg, because translucent heads show rows bleeding through during scroll). Drop scroll/maxHeight from the parent body Box — that layer becomes a flex/padding-only container that lets the scroll happen inside. Generalized rule: scroll boundary belongs at the layer that contains the sticky element. Putting scroll one layer outside causes the "header scrolls away" symptom every time.

### CR-020 — Implemented only the literal "auto-format" trigger instead of the implied "real editor experience"
- **Date**: 2026-05-15
- **Status**: active
- **Category**: assumption-error
- **Mode**: fix
- **What happened**: User asked `--fix lets this be with auto formater` for the Request body field. I interpreted "auto formatter" narrowly — added `onBlur` JSON pretty-print on top of a plain MUI multiline TextField. User came back with `this should be working as real editor`. The signal in the original request was "this field's editing UX feels broken, make it behave like the editor in my IDE" — auto-indent on newline, Tab inserts spaces, syntax highlighting, bracket matching. A plain `<TextField multiline>` with a blur-time format function is missing all of that; my fix solved one small part of a larger UX gap.
- **Root cause**: When the user says "auto formatter," that's the most concrete piece of a fuzzier request, but the screenshot showed broken indentation, no syntax color, and Tab moving focus instead of inserting whitespace — all classic plain-textarea pain points. I latched onto the named feature ("format") and didn't read the screenshot for what made it look unfinished. Pattern: when a user asks for a single named enhancement on an editing surface that's visibly under-featured, evaluate the whole field's UX gap before scoping the fix. A single feature add to a primitive textarea rarely answers "make this feel like a real editor."
- **Correct behavior**: For text fields that hold code/JSON/YAML/etc., default to "is this a code editor question?" before "is this a feature add?" If the field shows multi-line structured data, the right answer is usually a real editor component (CodeMirror, Monaco) with syntax highlighting, indentOnInput, bracketMatching, defaultKeymap — not a bag of handlers on a textarea. Pre-response self-check for body/payload/query/script-input fields: ask whether the field is structurally code (multi-line, has brackets/quoting rules, benefits from indent/highlight). If yes and the implementation is a plain `<TextField multiline>`, propose an editor swap, don't patch the textarea. The follow-up "this should be working as real editor" is a strong signal that confirms the scope.

### CR-019 — Recommended building a feature in answer to a yes/no design question instead of letting "no" be a first-class answer
- **Date**: 2026-05-15
- **Status**: active
- **Category**: assumption-error
- **Mode**: suggest
- **What happened**: User asked `--suggest is it good to show all client side api route in developer console`. I responded with "yes, but as a read-only inventory" and pitched a full v1 design (zone columns, source-of-truth strategy, sensitivity grouping) plus offered to scope the implementation. User replied `no need this feature`. The yes/no question got a 'yes' answer plus a built-in implementation pitch, which framed the conversation around "how to build it" instead of "is the underlying problem real."
- **Root cause**: I treated "is it good to show X?" as license to make the affirmative case and recommend a v1 scope. The question was actually a pre-commit gut-check — the user wanted me to evaluate whether the *problem this feature solves* exists for them, not to design the feature on the assumption that it does. By leading with the design and pushing toward "want me to scope v1?" I anchored the conversation in build-mode and put the burden on the user to push back. Compounded by suggest-mode's bias toward producing a substantial recommendation list — that bias is wrong when the right answer is "you probably don't need this."
- **Correct behavior**: For yes/no questions about adding a feature ("is it good to show X?", "should we add Y?", "is it worth doing Z?"), open with the case *against* first — what problem this would solve, whether that problem actually exists in the current workflow, who would use it weekly vs. once. Only then make the case for, and only at the user's prompt design the v1. Treat the "you probably don't need this" answer as just as helpful as the "here's how to build it" answer. Pre-response self-check on suggest-mode design questions: write one sentence stating who hits the problem this feature solves, and how often. If that sentence is speculative or low-frequency, lead with skepticism, not enthusiasm.

### CR-018 — Removed `useSoftDelete` from one file then flagged the rest as a follow-up, when the user wanted full project removal
- **Date**: 2026-05-14
- **Status**: active
- **Category**: incomplete-phase
- **Mode**: fix
- **What happened**: User said `--fix also remove softdelete hook as we no need that any more as backend has remove it`. I interpreted "softdelete hook" narrowly — removed `useSoftDelete` only from `inventory-balance-list-view.tsx` and replaced it with a per-entity `DELETE_INVENTORY_BALANCE` mutation. I left `useSoftDelete` in place in `delete-record-state.tsx`, the whole `packages/lib/src/soft-delete/` folder intact, and merely *flagged* the rest at the end of the response. User came back with `--fix useSoftDelete remove this permantly from our project` to force the full sweep.
- **Root cause**: When the user named a specific file in the *previous* turn ("inventory balance list page") and then said "backend has remove it" in a follow-up, I anchored on the previous file as the scope. The phrase "backend has remove it" is a global signal (the backend mutation is gone), not a file-scoped one — there's no per-file backend removal. If the backend mutation no longer exists, every call site is broken, so the scope is the whole project. Asking "this file only or everywhere?" up front would have caught it; defaulting to file-only was the wrong default for a phrase that implied global decommission.
- **Correct behavior**: When a fix involves a hook/mutation/import that is being decommissioned ("backend removed it", "we don't use X anymore", "remove pattern Y"), default to **project-wide removal** unless the user explicitly says "just this file". Map all consumers via `grep` first, present the surface (file count + breakdown of consumer types), then either propose a scoped subset or proceed with full removal. For decommission language specifically, never stop at one consumer and offload the rest as a "follow-up" — that's coward's incrementalism that leaves the project half-broken at compile time.

### CR-017 — Designed a sticky footer ribbon that duplicated the hero panel's CTAs + meter summary
- **Date**: 2026-05-13
- **Status**: active
- **Category**: style-drift
- **Mode**: implement, mockup
- **What happened**: When implementing the v1 mockup for `delete-record-state.tsx`, I included a sticky `position: fixed` footer ribbon at the bottom of the page that mirrored the hero panel's "Go back" + "Retry delete" CTAs and a tabular summary of `{cleared} of {total} dependencies cleared · Resolve all blockers to enable Retry Delete`. The user removed it with `--fix remove this footer, not needed`. The hero panel already shows the same meter (big numeric stat + remaining counter + gradient progress bar) and the same pair of CTAs (`Go back` ghost + `Retry delete` primary). Putting them again at the bottom-of-viewport ribbon was decorative duplication, not a workflow aid.
- **Root cause**: I imported the Linear/Stripe "always-reachable sticky CTA" pattern from the mockup playbook without checking whether the page actually needed it. That pattern earns its keep on long-form pages where the primary CTA scrolls off-screen and users lose the action affordance. On this page, the hero panel is at the top with its own CTAs, and the workspace below it is the user's focused work area — they're not "browsing through content looking for an action." The sticky footer was solving a problem that didn't exist, while consuming 60+ pixels of viewport on every screen and forcing a `pb: 14-16` clearance hack on the page container.
- **Correct behavior**: Before adding a sticky footer to a workflow page, ask: do the primary CTAs scroll off-screen during normal use? If the page fits in viewport (hero + workspace) or the CTAs already live in a fixed/sticky region (header, hero panel that stays visible during the relevant scroll), a sticky footer is redundant. Sticky footers are for long-scroll content (timelines, document editors, settings pages) where the user can lose the action surface. This page's hero panel is its action surface — re-pinning the same CTAs at the bottom is theme-matching the mockup pattern rather than serving the user. Pre-response self-check on premium-mockup translations: for each "sticky / floating / pinned" element, write one sentence why the user can't reach it from the natural top-of-page position; if that sentence is forced, drop the element.

### CR-016 — Re-violated LR-039 by littering `entity-map.ts` with section-divider banners and self-descriptive JSDoc on already-named functions/fields
- **Date**: 2026-05-13
- **Status**: [PROMOTED to LR-042 with refinement] — User then clarified on 2026-05-13 that function JSDocs documenting return-condition logic (`Returns null for non-routable entities`), input format examples (`e.g. "PurchaseOrder"`), and conceptual operations (`Resolve... to its map entry`) ARE acceptable. Over-correction reversed: function JSDocs restored in `entity-map.ts`; section dividers and file-level prose block kept removed. The refined acceptable-comment taxonomy is now LR-042.
- **Category**: style-drift
- **Mode**: fix
- **What happened**: When expanding `packages/constants/src/common/entity-map.ts` to cover all nav-config entities, I added: (a) a 13-line file-level block comment at the top describing what the map does, (b) section-divider banners (`// ── Transactions ──`, `// ── Master data ──`, `// ── Network ──`, `// ── Administration ──`, `// ── Policies (list-only) ──`, `// ── Crosswalk / mapping (list-only) ──`, `// ── Business objects (list-only) ──`, `// ── Inventory / products (list-only) ──`, `// ── Transactions (list-only, no detail page) ──`, `// ── Child / line-level tables (no standalone route) ──`, `// ── Lookup maps (built once at module load) ──`), and (c) one-line JSDoc on every interface field and every exported function — including `/** Whether this entity has a detail page. */`, `/** Resolve a backend tableName... */`, `/** Build the detail page path... */`, `/** Get all registered entity map entries. */` — all of which just restate the identifier name. The user said `--fix no need to add comments like what you doing, if you want to write comment when it should be like how we use it or logic behind it`. The pattern is identical to CR-014 (rationale comments) and CR-015 (multi-line `//` blocks) — I keep treating "this section helps navigation" or "this function has a return-value subtlety" as license to comment.
- **Root cause**: I treated comments as a documentation aid (section organization, return-value docs, type-field clarification) rather than the LR-039 contract — "only when the WHY is non-obvious and removing the comment would confuse a future reader." Section banners are organizational metadata that the reader can derive from scanning entries' `tableName` fields. Function JSDocs that say what the function does are restatements of the function name. Field JSDocs that say `routable: boolean → "Whether this entity has a detail page."` are pure restatement. The valid comments — format hints like "PascalCase" on `tableName`, "kebab-case" on `slug`, and the `detailPath` JSDoc that explains the fallback-when-omitted logic — are kept; everything else is noise.
- **Correct behavior**: Pre-response self-check needs a stricter pass on data-table files (entity maps, route maps, config arrays): grep for `// ──`, `/** Resolve`, `/** Build`, `/** Get all`, `/** Whether`, and the file-level `/**` block. Drop them. The only acceptable comments in this kind of file are: (1) format conventions ("PascalCase", "kebab-case") that the type system can't encode, (2) optional-field logic notes that explain fallback behavior, (3) `TODO(name):` or `eslint-disable-*`. Never section banners, never JSDoc on functions whose names are self-describing, never field JSDocs that restate the field. If the user says "comments should explain HOW to use or LOGIC behind" — apply that aggressively.

### CR-015 — Re-violated LR-039 by writing rationale comments across the nav-config consolidation files
- **Date**: 2026-05-13
- **Status**: Active (count = 1) — second re-violation of an already-promoted rule (CR-014 was the first this session)
- **Category**: style-drift
- **Mode**: fix
- **What happened**: While building the `packages/nav-config` package and wiring the multi-app role-route-access UI, I littered comments across `packages/nav-config/src/configs/types.ts` (3-line header), `packages/nav-config/src/configs/supplier-routes.ts` (3-line "Source of truth" block), `packages/nav-config/src/role-access/types.ts` (JSDoc on `apps?` field), `packages/nav-config/src/role-access/RouteGroupsTree.tsx` (4-line section header above `AppSourceTag`), `packages/ui/src/layouts/components/searchbar/utils.ts` (3-line JSDoc + 2-line `// Leaf items missing a path…` block), and especially `apps/scm/src/sections/components/route-access/nav-route-groups.ts` (six separate multi-line `//` blocks above each function explaining "source of truth", "dedupe rationale", "default precomputed groups: SCM only" etc). The user said `--fix still you are writing comments`. All comments I added in this session have been stripped.
- **Root cause**: Same pattern as CR-014 — when the code is "non-obvious" or "subtle" (cross-app merging, dedup-with-tag-preservation, structural-vs-React distinction), the temptation is to write a multi-line `//` block to explain. LR-039 explicitly forbids this; the explanation belongs in the chat summary / PR description, not the source. I also added JSDoc blocks on type fields (`apps?: string[]`) and section banners (`// App-source pill...`), treating those as exempt — they aren't.
- **Correct behavior**: Zero comments by default, period. JSDoc on exported types is comments. Banner separator comments (`// ----------`) above new code I add are comments. Multi-line `//` explanations are comments. The pre-response self-check needs a grep for `^\s*//` and `^\s*/\*\*` on files I just edited; if any line I added matches, strip it before declaring done. The acceptable exceptions remain: `TODO(name):`, `eslint-disable-*`, and a single-line non-obvious workaround flag. Nothing else. When tempted to "document a subtle invariant," write that in the chat summary instead — the reader of the PR sees both, but the future reader of the source sees only the code, and that's the LR-039 contract.

### CR-014 — Added multi-line rationale comments alongside permission-fix code edits, re-violating LR-039
- **Date**: 2026-05-13
- **Status**: Active (count = 1) — re-violation of an already-promoted rule
- **Category**: style-drift
- **Mode**: fix
- **What happened**: While applying the permission audit fixes to the supplier purchase-orders list view and the packaged-product form-container, I added narrative comments next to the permission code — three lines above the `onValid` mutation guard explaining defense-in-depth strategy, a two-line block above `permissionsByEntity` explaining why it was added, and a two-line block above the columns `useMemo` dep array explaining the static-canRead pattern. The user pushed back with `--fix no need to add comments`. All three comment groups have been removed.
- **Root cause**: LR-039 is unambiguous — default to zero comments. I treated the "why" of `permissionsByEntity` and the "defense-in-depth" of the form-container guard as non-obvious enough to deserve a comment. In practice the code itself plus the PR/commit description carries that intent. The slope I keep falling down is: "this is subtle, the next reader won't get it without context, let me add a note." That's the exact pattern LR-039 rejects — identifiers and code structure must carry the meaning, not multi-line `//` blocks.
- **Correct behavior**: When adding a permission guard, a Redux selector for memo reactivity, or any "subtle" code, just ship the code. The summary message back to the user is the right place for the rationale — not a comment in the file. The only inline notes allowed are `TODO(name): action`, `eslint-disable-next-line <rule>`, or a single-line non-obvious workaround flag (LR-039 exception (c)). The `useMemo` dep array with `permissionsByEntity` is exactly the kind of thing where the dep itself is the documentation; the explanation belongs in the PR. Apply this BEFORE the pre-response self-check too — adding comments and then expecting to catch them later violates the "comments rot" rationale because the catch can miss subtle ones.

### CR-013 — Treated the supplier app's `GET_DEV_ADMIN_APP_USER_BY_ID` call in `DashboardDynamicLayer` as a legitimate supplier feature instead of recognising it as a SCM-only check that leaked across
- **Date**: 2026-05-13
- **Status**: Active (count = 1)
- **Category**: assumption-error
- **Mode**: check, fix
- **What happened**: User asked why the supplier dashboard takes time to load. I produced a `--check` report flagging the serial SSR waterfall in `apps/supplier/src/app/(redux)/dashboard/dashboard-dynamic-layer.tsx` and suggested *parallelising* `setCurrentRoleInCookie` with the `GET_DEV_ADMIN_APP_USER_BY_ID` query — implicitly assuming the dev-admin query was a deliberate supplier-app dependency. User corrected: the supplier app does not need that query at all; it is a SCM-only concept and the call here is a copy-paste leak from `apps/scm/src/app/(redux)/dashboard/dashboard-dynamic-layer.tsx`. The fix is to delete the query and the `isDeveloper` branch entirely, not to parallelise it.
- **Root cause**: I read the supplier `DashboardDynamicLayer` in isolation and treated every query it ran as legitimate-by-existence. I did not compare against the SCM equivalent to ask "is this branch supplier-specific or copied unchanged from the other app?". The dev-admin flag has no consumer in supplier (no `DeveloperGuard`, no `<PermissionSandbox>` gating on `isDeveloper`, no UI surface that branches on it) — a quick `grep -rn "isDeveloper" apps/supplier/src` would have shown zero use sites and revealed the dead code path.
- **Correct behavior**: When a supplier-app file looks structurally identical to its SCM counterpart, before recommending any optimisation on it, run a quick `diff` mental pass: which branches in this file are exercised in *this* app, and which were copy-pasted from SCM and now dead-weight? Specifically for boot/bootstrap server components and `_app`-equivalent layers, every awaited query must have a downstream consumer in the same app — if `grep -rn "consumer-of-the-flag" apps/<this-app>/src` returns zero, the query is a deletion candidate, not an optimisation candidate. Applies broadly: any time SCM and supplier share a near-identical file, treat shared bootstrap calls as "guilty until proven needed" and verify consumer presence before suggesting a perf fix that preserves the call.

### CR-012 — Picked the wrong skeleton-loading fix: passed `mode: 'client'` to `useTableColumns` instead of switching the page to `CrudListPage` server mode with a `ServerSideDatasource`
- **Date**: 2026-05-12
- **Status**: Active (count = 1)
- **Category**: wrong-mode
- **Mode**: fix
- **What happened**: User reported family + brand list pages no longer showed loading skeletons after a prior commit removed per-column `customColumnConfig.renderer` blocks. I traced the cause to `useTableColumns` only injecting the skeleton-aware `cellRenderer` when called with `mode: 'client'`, and the two views defaulted to `'server'`. I added `mode: 'client'` to both `useTableColumns` calls — kept the existing `useQuery(GET_*_ROWS, { startRow: 0, endRow: 10000 })` "fetch everything up front" pattern and the `tableData`/`loading`/`skeletonColumnConfig` props on `CrudListPage`. User pushed back: `CrudListPage` already supports `serverSideDatasource`, the `getBrandRows`/`getFamilyRows` queries are SSRM-shaped (`RowRequest` input, `totalRows`/`rows` response), so the right move is to switch the views to server mode entirely — AG Grid's native loading overlay handles the visual, and `mode` on `useTableColumns` is irrelevant (server is its default).
- **Root cause**: When the local fix worked, I stopped at it instead of asking whether the broader page architecture was on the right path. The "fetch 0..10000 in a single up-front query then render in client mode" pattern is a workaround that contradicts how every other reference-data list page in this codebase is built (`AccountingItemListView`, all xwalk views, etc., all use `ServerSideDatasource`). I should have noticed the SSRM-shaped query name (`getBrandRows`/`getFamilyRows` with `RowRequest` input) and the existing `refetchQueries` (instead of `invalidate` per LR-007) as signals that the views were in a transitional/wrong state, then proposed the larger fix.
- **Correct behavior**: Before fixing a loading-state symptom on a list page, identify whether the page is in client mode or server mode and whether that matches the **shape of its query**. Signals a page should be in server mode: query name ends in `Rows` (e.g., `getBrandRows`, `getSalesOrderRows`); input shape is `RowRequest` (`startRow`/`endRow`/`filterModel`/`sortModel`/`groupKeys`); response is `{ totalRows, rows: [...] }`; page consumes via `useQuery(..., { variables: { params: { startRow: 0, endRow: 10000 } } })` — that final pattern is a tell-tale workaround. Action when those signals match but `CrudListPage` is in client mode: convert the page to `mode="server"` with `new ServerSideDatasource(client, QUERY, 'rootName', { fetchPolicy: 'cache-and-network', onTotalRows })`, swap `refetchQueries` for `invalidate: refreshGrid` (LR-007), and drop the `skeletonColumnConfig` (SSRM has its own loading visual). Reference template: `apps/scm/src/sections/product/accounting-items/view/accounting-item-list-view.tsx`. Do NOT patch the symptom by toggling `useTableColumns` `mode` — that locks in the workaround.
